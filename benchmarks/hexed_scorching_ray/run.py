...
def validate_backend(exact_sweep, backend_name: str, config: BenchmarkConfig) -> None:
    ...
    for index, coordinate in enumerate(workload.VALIDATION_CELLS):
        ...
        exact = common.cells_dict(exact_sweep)[coordinate]
...
def main() -> None:
    ...
    print("Benchmark: Hexed/Blessed Scorching Ray")
    print("Axis order: {}".format(", ".join(workload.AXIS_ORDER)))
    print("Timing scope: full exact sweep across {:,} cells".format(len(tuple(workload.coordinate_space()))))
    print(
    ...
    exact_cells = common.cells_dict(exact_sweep)
    actual_axis_order = tuple(axis.name for axis in exact_sweep.axes)
    if actual_axis_order != workload.AXIS_ORDER:
        raise SystemExit("unexpected exact axis order {} (expected {})".format(actual_axis_order, workload.AXIS_ORDER))
    ...
    table_rows = [
        (
            "dice exact",
            "-",
            "full sweep ({:,} cells)".format(len(exact_cells)),
    ...
    for backend_index, (backend_name, sample_counts) in enumerate(benchmark_rows(config), start=1):
        ...
        for sample_index, sample_count in enumerate(sample_counts, start=1):
            ...
            error = common.mean_absolute_error(exact_sweep, sampled_sweep, workload.REPRESENTATIVE_CELLS)
            table_rows.append(
                (
    ...
    for label, sampled_sweep in sampled_runs.items():
        print("  {}:".format(label))
